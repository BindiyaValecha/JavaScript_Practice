let num = prompt("Enter the number ");
num = Number(num);

console.log(num,"x 1 =",num*1)
console.log(num,"x 2 =",num*2)
console.log(num,"x 3 =",num*3)
console.log(num,"x 4 =",num*4)
console.log(num,"x 5 =",num*5)
console.log(num,"x 6 =",num*6)
console.log(num,"x 7 =",num*7)
console.log(num,"x 8 =",num*8)
console.log(num,"x 9 =",num*9)
console.log(num,"x 10 =",num*10)



let num = prompt("Enter the number: ");
num = Number(num);

console.log(num,"x 1 =",num*1)
console.log(num,"x 2 =",num*2)
console.log(num,"x 3 =",num*3)
console.log(num,"x 4 =",num*4)
console.log(num,"x 5 =",num*5)
console.log(num,"x 6 =",num*6)
console.log(num,"x 7 =",num*7)
console.log(num,"x 8 =",num*8)
console.log(num,"x 9 =",num*9)
console.log(num,"x 10 =",num*10)



//conditional statement

// if(boolean){ // boolean true code run thendo na ta oho na thendo

//}



// if ("true" == "true1"){
//   console.log("Hello word!")
//   console.log(2+2);
//   let firstName = "avinash";
//   console.log(firstName);
  
// }

// console.log("hello pakistan");

// if (false){
//   console.log("turn");
//   let harry = "jack";
//   console.log("harry")
// }


  

  
// if (2 <= 9){
//   console.log("carry")
//   let sobh = "fame";
//   console.log(sobh)
// }




// let num1 = prompt("enter your number");
// let num2 = prompt("enter your number");
// console.log (num1," is greater than ",num2 ,num1 > num2)
// console.log (num1, " is less than", num2 , num1 < num2)
// console.log (num1, "greater than or equal to", num2, num1  >= num2)
// console.log (num1, "less than or equal to", num2, num1 <= num2)





if (true == true){
  console.log ("7>=7")
  let promo = 33
  console.log (promo)
}










